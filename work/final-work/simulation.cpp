...
ground simulation::g = { Vector3(0, 1, 0), 0.0f };
/* constructor */
...
void simulation::collisions(sphere* &s1,  sphere* & s2) {
    ...
    if (s1->detect(g.ground_level) == 1) {
        s1->res(g.ground_normal);
    }
...
void simulation::collisions(cube*& c1, cube*& c2, sphere*& s1, sphere*& s2) {
    if (c1->detect(g.ground_level) == 1) {
        c1->res(g.ground_normal);
    }
    if (c1->detect(*c2) == 1) { // detect collision with other sphere
        c1->res(*c2); // resolve collision with other sphere
    }
    if (c1->detect(*s1) == 1) { // detect collision with other sphere
        c1->res(*s1); // resolve collision with other sphere
    }
    if (c1->detect(*s2) == 1) { // detect collision with other sphere
...
void simulation::drawCube(cube* c) {
    glPushMatrix();
    glColor3f(1, 0, 0);
    glTranslatef(c->drawposition().getX(),c->drawposition().getY(), c->drawposition().getZ());
    glutSolidCube(c->getRadius()*2);
    glPopMatrix();
}
...
void simulation::reshape(int w, int h){
    ...
    glMatrixMode(GL_MODELVIEW);
    glLoadIdentity();
    gluLookAt(0, 0, 10, 0, 0, 0, 0, 1, 0); // position camera 
