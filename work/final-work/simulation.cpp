...
void simulation::drawSphere() {
    ...
    for (int i = 0; i < s.size(); i++) {
        glPushMatrix();
        glColor3f(1, 0, 0);
        glTranslatef(s[i]->getPosition().getX(), s[i]->getPosition().getY(), s[i]->getPosition().getZ());
        glutSolidSphere(s[i]->getRadius(), 20, 20); // stacks and slices
        glPopMatrix();
    }
...
void simulation::drawCube() {
    ...
    for (int i = 0; i < c.size(); i++) {
        glPushMatrix();
        glColor3f(1, 0, 0);
        glTranslatef(c[i]->getPosition().getX(), c[i]->getPosition().getY(), c[i]->getPosition().getZ());
        glutSolidCube(c[i]->getRadius() * 2);
        glPopMatrix();
    }
    /* push and pop matrix so cubes are drawn independently
           so they dont appear to overlap or intersect */
}
